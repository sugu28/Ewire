                                    ...
                                    <table class="table" style="width:100%">
                                        ...
                                        <tbody>
                                            ...
                                            if ($_SESSION['User_RoleID'] == 1) {
                                                $res = mysqli_query($db, "SELECT tblQRCodeCashWithdrawal.* FROM tblQRCodeCashWithdrawal");
                                            } elseif ($_SESSION['User_RoleID'] == 2) {
                                                $res = mysqli_query($db, "SELECT tblQRCodeCashWithdrawal.* FROM tblQRCodeCashWithdrawal INNER JOIN tblRetailer ON tblQRCodeCashWithdrawal.kioskid = tblRetailer.kioskId INNER JOIN tblMasterDistributor ON tblRetailer.MAsterDistributorID = tblMasterDistributor.Id 
                                                WHERE tblMasterDistributor.UserID = '$uid' ORDER BY tblQRCodeCashWithdrawal.creditedon DESC");
                                            } elseif ($_SESSION['User_RoleID'] == 3) {
                                                $res = mysqli_query($db, "SELECT tblQRCodeCashWithdrawal.* FROM tblQRCodeCashWithdrawal INNER JOIN tblRetailer ON tblQRCodeCashWithdrawal.kioskid = tblRetailer.kioskId INNER JOIN tblDistributor ON tblRetailer.DistributorID = tblDistributor.Id 
                                                WHERE tblDistributor.UserID = '$uid' ORDER BY tblQRCodeCashWithdrawal.creditedon DESC");
                                            } elseif ($_SESSION['User_RoleID'] == 4) {
                                                $res = mysqli_query($db, "SELECT * FROM tblQRCodeCashWithdrawal WHERE kioskid = (SELECT Kioskid FROM tblRetailer WHERE UserID = '$uid') ORDER BY tblQRCodeCashWithdrawal.creditedon DESC");
                                            }
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;"><?php echo $row["creditedamount"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["creditedon"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["mobilenumber"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["impsrefno"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["dispencetype"]; ?></td>
