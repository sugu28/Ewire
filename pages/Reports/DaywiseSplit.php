                                    ...
                                    <table class="table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="text-align: center;">NO_OF_TXN-KIOSK</th>
                                                <th style="text-align: center;">NO_OF_TXN-YAPAY</th>
                                                <th style="text-align: center;">NO_OF_TXN_FAILED</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            if ($_SESSION['User_RoleID'] == 1) {
                                                $res = mysqli_query($db, "SELECT kioskid,DATE_FORMAT (trans_datetime,'%d-%m-%Y') AS DATE,SUM(amount) AS AMOUNT,
                                                SUM(Is_journal_done) AS NO_OF_TXN_KIOSK,SUM(Is_yapay_done) AS NO_OF_TXN_YAPAY,SUM(Is_trans_failed) AS NO_OF_TXN_FAILED
                                                FROM tbl_Transaction_Status WHERE Is_yapay_done = '1' GROUP BY  kioskid, DATE(trans_datetime) DESC");
                                                ...
                                                $res = mysqli_query($db, "SELECT kioskid,DATE_FORMAT (trans_datetime,'%d-%m-%Y') AS DATE,SUM(amount) AS AMOUNT,
                                                SUM(Is_journal_done) AS NO_OF_TXN_KIOSK,SUM(Is_yapay_done) AS NO_OF_TXN_YAPAY,SUM(Is_trans_failed) AS NO_OF_TXN_FAILED
                                                FROM tbl_Transaction_Status WHERE Is_yapay_done = '1' AND tbl_Transaction_Status.kioskid = '$kioID' GROUP BY DATE(trans_datetime) DESC");
                                            }
                                            $cnt = 1;
                                            while ($row = mysqli_fetch_array($res)) { ?>
                                                <tr>
                                                    ...
                                                    <td style="text-align: center;"><?php echo $row["NO_OF_TXN_KIOSK"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["NO_OF_TXN_YAPAY"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["NO_OF_TXN_FAILED"]; ?></td>
                                                </tr>
                                            <?php
                                            }
                                            ?>
