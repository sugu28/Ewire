                                    ...
                                    <table class="table" style="width:100%">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="text-align: center;">actualtransferamount</th>
                                                <th style="text-align: center;">Transaction ref_no</th>
                                                <?php
                                                if ($_SESSION['User_RoleID'] == 2) {
                                                ?>
                                                    <th style="text-align: center;">DMT_Charges</th>
                                                    <th style="text-align: center;">Dist_Com</th>
                                                <?php } ?>
                                                <th style="text-align: center;">KIOSK_Com</th>
                                                <th style="text-align: center;">Retailer_TDS_Charges</th>
                                        ...
                                        <tbody>
                                            ...
                                            if (isset($_GET['from_date']) && isset($_GET['to_date'])) {
                                                ...
                                                $filterQry = "AND Created_Date BETWEEN '$from_date' AND '$tt'";
                                            }
                                            if ($_SESSION['User_RoleID'] == 1) {
                                                $res = mysqli_query($db, "SELECT * FROM tbl_Commission_Details INNER JOIN tblCyberPlatCashTransfer ON tblCyberPlatCashTransfer.ref_no = tbl_Commission_Details.Ref_no INNER JOIN tblRetailer ON tblRetailer.Id = tbl_Commission_Details.RetailerID " . $filterQry . "ORDER BY tbl_Commission_Details.Created_Date DESC ");
                                            } elseif ($_SESSION['User_RoleID'] == 2) {
                                                $res = mysqli_query($db, "SELECT * FROM tbl_Commission_Details INNER JOIN tblCyberPlatCashTransfer ON tblCyberPlatCashTransfer.ref_no = tbl_Commission_Details.Ref_no INNER JOIN tblRetailer ON tblRetailer.Id = tbl_Commission_Details.RetailerID INNER JOIN tblMasterDistributor ON 
                                                tblRetailer.MasterDistributorID = tblMasterDistributor.Id WHERE tblMasterDistributor.UserID = '$uid' " . $filterQry . "ORDER BY tbl_Commission_Details.Created_Date DESC");
                                            } elseif ($_SESSION['User_RoleID'] == 3) {
                                                $res = mysqli_query($db, "SELECT tblCyberPlatCashTransfer.* FROM tblCyberPlatCashTransfer INNER JOIN tblRetailer ON tblCyberPlatCashTransfer.kioskid = tblRetailer.kioskId INNER JOIN tblDistributor ON tblRetailer.DistributorID = tblDistributor.Id WHERE tblDistributor.UserID = '$uid' " . $filterQry . " ORDER BY tblCyberPlatCashTransfer.trans_datetime DESC");
                                            } elseif ($_SESSION['User_RoleID'] == 4) {
                                                $res = mysqli_query($db, "SELECT * FROM tbl_Commission_Details INNER JOIN tblCyberPlatCashTransfer ON 
                                tblCyberPlatCashTransfer.ref_no = tbl_Commission_Details.Ref_no INNER JOIN tblRetailer ON
                                tblRetailer.Id = tbl_Commission_Details.RetailerID WHERE tblCyberPlatCashTransfer.kioskid = '$kioID' " . $filterQry . "ORDER BY tbl_Commission_Details.Created_Date DESC ");
                                            }



                                            //                                               $ret=mysqli_query($db,"SELECT * FROM tbl_Commission_Details INNER JOIN tblCyberPlatCashTransfer ON
                                            //                                                tblCyberPlatCashTransfer.ref_no = tbl_Commission_Details.Ref_no INNER JOIN tblRetailer ON tblRetailer.Id = tbl_Commission_Details.RetailerID");
                                            $cnt = 1;
                                            while ($row = mysqli_fetch_array($res)) { ?>
                                                ...
                                                <tr>
                                                    ...
                                                    <?php if ($_SESSION['User_RoleID'] == 2) { ?>
                                                        <td style="text-align: center;"><?php echo $row["DMT_Charges"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Dist_Com"]; ?></td>
                                                    <?php } ?>
                                                    <td style="text-align: center;"><?php echo $row["KIOSK_Com"]; ?></td>
                                                    <td style="text-align: center;"><?php echo $row["Retailer_TDS_Charges"]; ?></td>
