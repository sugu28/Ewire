                ...
                <div class="content-wrapper">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <form action="CashInMachine.php" method="POST">
                                        <table class="table" style="width:100%">
                                            <thead>
                                                <tr>
                                                    <!-- <th style="text-align: center;">SI No.</th> -->
                                                    <?php if ($_SESSION['User_RoleID'] == 2) { ?>
                                                        <th style="text-align: center;">KioskID</th>
                                                    <?php } ?>
                                                    <th style="text-align: center;">Cash_2000</th>
                                                    <th style="text-align: center;">Cash_500</th>
                                                    <th style="text-align: center;">Cash_200</th>
                                                    <th style="text-align: center;">Cash_100</th>
                                                    <th style="text-align: center;">Cash_50</th>
                                                    <th style="text-align: center;">Cash_20</th>
                                                    <th style="text-align: center;">Cash_10</th>
                                                    <?php if ($_SESSION['User_RoleID'] == 4) { ?>
                                            ...
                                            <tbody>
                                                ...
                                                if ($_SESSION['User_RoleID'] == 1) {
                                                    $res = mysqli_query($db, "SELECT SUM(received_2000_count) AS Cash_2000,SUM(received_500_count) AS Cash_500,
                                                    SUM(received_200_count) AS Cash_200,SUM(received_100_count) AS Cash_100,SUM(received_50_count) AS Cash_50,
                                                    SUM(received_20_count) AS Cash_20,SUM(received_10_count) AS Cash_10 FROM tblAcceptorCashAudit 
                                                    WHERE kioskid='$kioID' AND transdatetime BETWEEN '$filterQry' AND NOW() GROUP BY tblAcceptorCashAudit.kioskid");
                                                } elseif ($_SESSION['User_RoleID'] == 2) {

                                                    $res1 = mysqli_query($db, "SELECT CreatedOn FROM tbl_cash_out INNER JOIN tblRetailer ON tbl_cash_out.kioskid = tblRetailer.kioskId
                                                    INNER JOIN tblMasterDistributor ON tblRetailer.MasterDistributorID = tblMasterDistributor.Id 
                                                    WHERE tblMasterDistributor.UserID = '$uid' ORDER BY tbl_cash_out.CreatedOn DESC");
                                                    while ($row = mysqli_fetch_array($res1)) {
                                                        $filterQry = $row["CreatedOn"];
                                                    }

                                                    $res = mysqli_query($db, "SELECT *,SUM(received_2000_count) AS Cash_2000,SUM(received_500_count) AS Cash_500,
                                                    SUM(received_200_count) AS Cash_200,SUM(received_100_count) AS Cash_100,SUM(received_50_count) AS Cash_50,
                                                    SUM(received_20_count) AS Cash_20,SUM(received_10_count) AS Cash_10 FROM tblAcceptorCashAudit 
                                                    INNER JOIN tblRetailer ON tblAcceptorCashAudit.kioskid = tblRetailer.kioskId
                                                    INNER JOIN tblMasterDistributor ON tblRetailer.MasterDistributorID = tblMasterDistributor.Id 
                                                    WHERE tblMasterDistributor.UserID = '$uid' AND transdatetime BETWEEN '$filterQry' AND NOW() GROUP BY tblAcceptorCashAudit.kioskid");
                                                } elseif ($_SESSION['User_RoleID'] == 3) {

                                                    $res1 = mysqli_query($db, "SELECT CreatedOn FROM tbl_cash_out WHERE KioskID = '$kioID' ORDER BY tbl_cash_out.CreatedOn DESC LIMIT 1");
                                                    while ($row = mysqli_fetch_array($res1)) {
                                                        $filterQry = $row["CreatedOn"];
                                                    }
                                                    ...
                                                    SUM(received_200_count) AS Cash_200,SUM(received_100_count) AS Cash_100,SUM(received_50_count) AS Cash_50,
                                                    SUM(received_20_count) AS Cash_20,SUM(received_10_count) AS Cash_10 FROM tblAcceptorCashAudit 
                                                    WHERE kioskid='$kioID' AND transdatetime BETWEEN '$filterQry' AND NOW()");
                                                }
                                                $cnt = 1;
                                                while ($row = mysqli_fetch_array($res)) { ?>
                                                    <tr>
                                                        <!-- <td style="text-align: center;"><?php echo $row["Id"]; ?></td> -->
                                                        <?php if ($_SESSION['User_RoleID'] == 2 || $_SESSION['User_RoleID'] == 1) { ?>
                                                            <td style="text-align: center;"><?php echo $row["kioskid"]; ?></td>
                                                        <?php } ?>
                                                        <td style="text-align: center;"><?php echo $row["Cash_2000"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Cash_500"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Cash_200"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Cash_100"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Cash_50"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Cash_20"]; ?></td>
                                                        <td style="text-align: center;"><?php echo $row["Cash_10"]; ?></td>
                                                        <?php if ($_SESSION['User_RoleID'] == 4) { ?>
                                                            <td style="text-align: center;">
                                                                <button name="CashOut" class='btn-edit btn btn-square btn-sm btn-outline-info btn-icon-text' value="<?php echo $row["Cash_2000"] . "," . $row["Cash_500"] . "," . $row["Cash_200"] . "," . $row["Cash_100"] . "," . $row["Cash_50"] . "," . $row["Cash_20"] . "," . $row["Cash_10"] ?>"> Cash Out </button>
                                                            </td>
                                                        <?php } ?>
                                                    </tr>
                                                <?php
...
if (isset($_POST['CashOut'])) {
    ...
    $str_arr = explode(",", $ID);
    $kioID =  $_SESSION['User_KioID'];
    $query    = "INSERT INTO `tbl_cash_out`(KioskID,Cash_2000,Cash_500,Cash_200,Cash_100,Cash_50,Cash_20,Cash_10) 
                 VALUES ('$kioID','$str_arr[0]','$str_arr[1]','$str_arr[2]','$str_arr[3]','$str_arr[4]','$str_arr[5]','$str_arr[6]')";
    $result   = mysqli_query($db, $query);
    if ($result > 0) {
        echo "<script>Updated();</script>";
        echo "<script>location.href='CashInMachine.php'; </script>";
    }
}
?>